...
CObsSeq::CObsSeq(CObs *obsType, char **sequences, int nbObs, int nbDimensions)
// Constructor reading observation sequences from file
{
	...
	for(i=1;i<=mNbSequences;i++){
		...
		for(j=1; j <=T; j++){
			mObs[i][j]->SetSequence(sequences[j]);
		}
	}
#if 1
	double averageT = mObsCount/nbObs;
	cout << "Average T = " << averageT << endl << endl;
#endif	
	m1DArray = new CObs*[mObsCount+1];
	Make1DArray();
