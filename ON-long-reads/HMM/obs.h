...
public:
  ...
  virtual void SetSequence(char*) = 0;
  virtual void SetSequence(READ*) = 0;
...
public:
  ...
  void SetSequence(char *sequence){mObs = sequence[1];};
  void SetSequence(READ*){};
...
public:
  CVectorObs(int dim){mDimension = dim; mVect = SetCharVector(dim);};
  CVectorObs(CObs& obs){
		CVectorObs* obs2 = (CVectorObs*)(&obs);
  		mDimension = obs2->GetDim();
	   mVect = SetCharVector(mDimension);
  ...
  inline char* GetVect(void){return mVect;};
  ...
  void SetSequence(char *sequence) {
	...
	if(sequence[i]!='\0')
		mVect[i] = sequence[i];
	else { mVect[i] = '\0'; return;}
     };
  void SetSequence(READ *nbread) {};
