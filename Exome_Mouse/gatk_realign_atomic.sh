...
if [[ $CHR == "20" ]]		##Mouse : 20=X,
    then
    CHR="X"
fi
if [[ $CHR == "21"  ]]		    ##Mouse : 21=Y,
    then
    CHR="Y"
fi
if [[ $CHR == "22"  ]]           ##Mouse : 22=M,
    then
    CHR="M"
fi
...
echo $CHR
# $GATK -L 3 -I $INP -R $REF -T IndelIntervals -U -S SILENT -o indelIntv.txt;
...
# wait for 6 seconds if not complete
   ...
   while [[ $completed != "" ]];
      do 
      completed=`wc -l $status | awk '{print $1}'`
      if [[ $completed == "22" ]]; then  # all chromosomes completed, proceed to the next step (merge), 
...
#		  $cmd
     ...
     elif [[ $completed -lt "22" || $completed -gt "22" ]]; then exit;
     else sleep 20
     fi
    done
fi
