...
CHR="$INP.target.interval_list"

    
# if [[ $REFTYPE == "hg" ]]  # hg18/19 -> chr1, chr2 etc;  build36/37 -> 1, 2 etc                                            
#    then
    cat $ExonFile | awk '{ print $1":"$2"-"$3}' > $CHR #"$INP.target.list"
# else
#    cat $ExonFile | awk '{print $1":"$2"-"$3}' | sed 's/chr//' > $CHR # "$INP.target.list"
# fi
echo $REF
echo  $INP
echo  $CHR
...
then
	...
	cmd_varcalling="sh ${BPATH}/joint_SNV-indel_calling-split-by-intervals.sh -i $INP.list -m 8 -s $GLOBAL -n 1 -j 100 -d 300 -v 10 -t 30 -o $path_inp/VarCalling -A AUTO "
	echo $cmd_varcalling
	$cmd_varcalling	
fi
