...
if [[ $chain != ""  ]]; then
    ...
    date
	if [[ $AUTO == "" ]]; then
	    cmd="qsub -N depth.$job_name -l mem=8G,time=48:: -o $OUTPUT.depth.log.o -e $OUTPUT.depth.log.e ${BPATH}/gatk_depthofcoverage.sh  -g $GLOBAL -I $OUTPUT  -m 7000 "
	else
	    cmd="qsub -N depth.$job_name.AUTO -l mem=8G,time=48:: -o $OUTPUT.depth.log.o -e $OUTPUT.depth.log.e ${BPATH}/gatk_depthofcoverage.sh  -g $GLOBAL -I $OUTPUT  -m 7000 -A AUTO"
	fi
    echo $cmd
    $cmd
