...
File.new(infile, 'r').each do |line|
   ...
   if ref.size == alt.size and (not ref.include? "-")  and (not alt.include? "-")  and  ref.size == 1 and alt.size == 1  # it is SNV
	...
	if infoall.include? "function=exonic"
		...
		codingFrameDel += 1 if infoall.include? "functionalClass=frameshiftdeletion"
		codingFrameIns += 1 if infoall.include? "functionalClass=frameshiftinsertion"
		codingNonFrameDel += 1 if infoall.include? "functionalClass=nonframeshiftdeletion"
		codingNonFrameIns += 1 if infoall.include? "functionalClass=nonframeshiftinsertion"
		codingNonFrameSub += 1 if infoall.include? "functionalClass=nonframeshiftsubstitution"
		codingMissense += 1 if infoall.include? "functionalClass=nonsynonymousSNV"
		codingNonsense += 1 if infoall.include? "functionalClass=stopgainSNV"
		codingReadthrough += 1 if infoall.include? "functionalClass=stoplossSNV"
		codingSynonymous += 1 if infoall.include? "functionalClass=synonymousSNV"
	end
