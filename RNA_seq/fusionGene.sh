...
script_PE="$RNABASE/tophatFusion.sh"
...
if [[ $f2 == "" ]]; then
    ...
    qsub -o logs/pipe.$f1_base.o -e logs/pipe.$f1_base.e -pe smp 4 -R y -l mem=6G,time=100:: -N do.$f1_base $script_PE  -f $f1 -r $f2 -s $setting_genome -n 4 -o "cufflinks/"$f1_base"_"$genome"_pe_fusion"
fi
