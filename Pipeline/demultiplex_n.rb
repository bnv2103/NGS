...
def doSplit(bfq, targetfq, lane, multiplex, outprefix, barcodesize)
  ...
  multiplex[lane].values.sort.each do |sampleID|
    outName = outprefix + "_lane#{lane}n_#{sampleID}_#{ends}.fastq"
    outio[sampleID] = File.new(outName,'w')
    reads_sample[sampleID] =0;
 end
  
  outio[:discard] = File.new(outprefix + "_lane#{lane}n_#{ends}.unknown.fastq", 'w')
...
#   Zlib::GzipReader.new
 ...
 ### Output summary of reads per sample and reads per lane etc.
  ...
  read_summary_file =  File.open(outprefix + "_summary_lane#{lane}n.stats",'a+')
  lane_out="lane"
  temp_out="#{lane}"
  $reads_lane[lane] = []
